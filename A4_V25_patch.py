...
new_block = r'''        def publish_guard_status(event: dict[str, Any]) -> None:
        ...
        def repair_runtime(reason: str) -> dict[str, Any]:
            ...
            # Preserve full-market throughput.  A 1975-symbol cycle needs the normal
            # worker floor; reducing to 1-2 workers guarantees deadline misses.
            ...
            # A previous transport failure must not leave a global backoff poison
            # behind after the session pool has been rebuilt.
            ...
            return {
                ...
                "session_pool_size": core.SESSION_POOL_SIZE,
                "retry_trigger_coverage": core.RETRY_TRIGGER_COVERAGE,
                "warmup_ok_count": sum(1 for x in warm if x.get("ok")),
                "warmup_total": len(warm),
            }
        ...
        def guarded_capture(stocks: list[Any], cycle_deadline: float):
            ...
            if cycle_ok:
                ...
                guard_state["consecutive_bad_cycles"] += 1
            record = {
                "mono": time.monotonic(), "coverage": coverage, "required_coverage": required_coverage,
                "attempts": attempts, "errors": errors, "error_rate": error_rate,
                "elapsed_seconds": elapsed, "integrity": dict(integrity),
                "accepted_symbols": len(accepted_codes), "requested_symbols": len(expected_codes),
                "missing_symbols": sorted(expected_codes - accepted_codes)[:100], "cycle_ok": cycle_ok,
                "consecutive_bad_cycles": int(guard_state["consecutive_bad_cycles"]),
            }
            ...
            if audit_due:
                ...
                coverages = sorted(float(x["coverage"]) for x in window)
                avg_coverage = sum(coverages) / len(coverages) if coverages else 0.0
                median_coverage = coverages[len(coverages)//2] if coverages else 0.0
                min_coverage = min(coverages, default=0.0)
                integrity_totals = Counter()
                for x in window:
                    integrity_totals.update(x.get("integrity", {}))
                good_cycles = sum(1 for x in window if x.get("cycle_ok"))
                bad_cycles = len(window) - good_cycles
                good_ratio = good_cycles / len(window) if window else 0.0
                # One isolated transport wobble is observed but must not trigger a
                # self-amplifying pool/concurrency collapse.  Persistent failures do.
                ...
                if window_ok:
                    ...
                    guard_state["healthy_windows"] += 1
                    status = "STABLE" if int(guard_state["healthy_windows"]) >= 3 else "PASS_WARMING"
                    write_guard({
                        "status": status, "profile": guard_state["profile"], "repairs": guard_state["repairs"],
                        "healthy_windows": guard_state["healthy_windows"], "window_cycles": len(window),
                        "good_ratio": good_ratio, "bad_cycles": bad_cycles, "average_coverage": avg_coverage,
                        "median_coverage": median_coverage, "minimum_coverage": min_coverage,
                        "request_error_rate": window_error_rate, "integrity": dict(integrity_totals),
                    })
                elif int(guard_state["consecutive_bad_cycles"]) < 2:
                    guard_state["healthy_windows"] = 0
                    write_guard({
                        "status": "DEGRADED_OBSERVE", "profile": guard_state["profile"], "repairs": guard_state["repairs"],
                        "healthy_windows": 0, "consecutive_bad_cycles": guard_state["consecutive_bad_cycles"],
                        "window_cycles": len(window), "good_ratio": good_ratio, "bad_cycles": bad_cycles,
                        "average_coverage": avg_coverage, "median_coverage": median_coverage,
                        "minimum_coverage": min_coverage, "request_error_rate": window_error_rate,
                        "integrity": dict(integrity_totals),
                    })
                else:
                    guard_state["healthy_windows"] = 0
                    ...
                    elif window_error_rate > RUNTIME_MAX_ERROR_RATE:
                        reason = "TRANSPORT_ERROR"
                    elif median_coverage < required_coverage:
                        reason = "LOW_COVERAGE"
                    else:
                        reason = "SOURCE_GAP"
                    recovery = None
                    if int(guard_state["repairs"]) < RUNTIME_MAX_REPAIRS:
                        recovery = repair_runtime(reason)
                        status = "AUTO_REPAIRED"
                    else:
                        status = "FAIL_CLOSED" if int(guard_state["bad_windows"]) >= RUNTIME_FAIL_CLOSED_WINDOWS else "DEGRADED_OBSERVE"
                    write_guard({
                        "status": status, "reason": reason, "profile": guard_state["profile"],
                        "repairs": guard_state["repairs"], "healthy_windows": 0,
                        "bad_windows": guard_state["bad_windows"], "window_cycles": len(window),
                        "good_ratio": good_ratio, "bad_cycles": bad_cycles, "average_coverage": avg_coverage,
                        "median_coverage": median_coverage, "minimum_coverage": min_coverage,
                        "request_error_rate": window_error_rate, "integrity": dict(integrity_totals),
                        "recovery": recovery,
...
# Final formal acceptance requires three healthy 30-second windows.
...
assert old in s, 'formal acceptance block not found'
s = s.replace(old, new, 1)
cp.write_text(s, encoding='utf-8')

wf = Path('.github/workflows/scraper.yml')
w = wf.read_text(encoding='utf-8')
needle = '      DEFAULT_BRANCH: ${{ github.event.repository.default_branch }}\n\n    steps:'
repl = '      DEFAULT_BRANCH: ${{ github.event.repository.default_branch }}\n      GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}\n\n    steps:'
assert needle in w, 'production env marker missing'
