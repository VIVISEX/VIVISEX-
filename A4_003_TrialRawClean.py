...
def _combine_market_dt(d: date | None, t: time | None) -> datetime | None:
    return datetime.combine(d, t, TZ) if d is not None and t is not None else None
...
def _levels_from_twse_item(item: dict[str, Any]) -> dict[str, Any]:
    def split5(key: str, integer: bool = False) -> list[Any]:
        raw = item.get(key)
        vals = [] if raw is None else [x.strip() for x in str(raw).split("_") if x.strip()]
        converted = [_num(v, integer=integer) for v in vals[:5]]
        return converted + [None] * (5 - len(converted))

    bp, bv = split5("b"), split5("g", True)
    ap, av = split5("a"), split5("f", True)
    out: dict[str, Any] = {}
    for i in range(5):
        n = i + 1
...
def _detect_source(record: dict[str, Any]) -> str:
    hinted = str(record.get("source") or "").strip().upper()
    if hinted in {"TWSE", "TWSE_MIS", "MIS"}:
        return "TWSE_MIS"
    if hinted == "SHIOAJI":
        return "SHIOAJI"
    if isinstance(record.get("raw"), dict) or "msgArray" in record or any(k in record for k in ("b", "a", "g", "f", "ts")):
        return "TWSE_MIS"
...
def expand_input(record: dict[str, Any]) -> list[dict[str, Any]]:
    source = _detect_source(record)
    if source == "TWSE_MIS" and isinstance(record.get("raw"), dict):
        envelope = record
        out = [
            {"__source": "TWSE_MIS", "__envelope": envelope, "__item": item}
            for item in (envelope["raw"].get("msgArray", []) or [])
        ]
    ...
    if source == "TWSE_MIS" and "msgArray" in record:
        return [{"__source": "TWSE_MIS", "__envelope": record, "__item": item} for item in (record.get("msgArray") or [])]
    return [{"__source": source, "__envelope": record, "__item": record}]


def normalize_record(expanded: dict[str, Any], *, run_id: str, expected_as_of_date: date | None = None) -> dict[str, Any]:
    ...
    source_gap = (
        str(envelope.get("dq_status", "")).upper() == "SOURCE_GAP"
        or str(item.get("dq_status", "")).upper() == "SOURCE_GAP"
    )
    ...
    dq: list[str] = []
    if source == "UNKNOWN": dq.append("UNKNOWN_SOURCE")
    if source_gap: dq.append("SOURCE_GAP")
    if not code and not source_gap: dq.append("MISSING_SYMBOL")
    if event_at is None and not source_gap: dq.append("INVALID_MARKET_TIMESTAMP")
    if available_at is None: dq.append("INVALID_AVAILABLE_AT")
    if market_date and as_of and market_date > as_of: dq.append("LOOKAHEAD_VIOLATION")
    ...
    book_fields = [levels[f"{side}_{kind}_{i}"] for side in ("bid", "ask") for kind in ("price", "volume") for i in range(1, 6)]
    book_complete = all(v is not None for v in book_fields)
    if phase == "PREOPEN" and not source_gap and not book_complete: dq.append("PARTIAL_BOOK")
    ...
    key_material = "|".join([source, source_date or "N/A", code or "N/A", event_iso or "N/A", available_iso or "N/A"])
    data_key = hashlib.sha256(key_material.encode()).hexdigest()

    row = {
        "DATA_KEY": data_key, "SOURCE": source, "SOURCE_DATE": source_date,
        "AVAILABLE_AT": available_iso, "AS_OF_DATE": as_of_date, "VERSION": VERSION,
        "RUN_ID": run_id, "STRATEGY_ID": STRATEGY_ID, "FUND_ID": FUND_ID,
        "event_at": event_iso, "phase": phase, "market": market, "code": code or None,
        "simulated": simulated, "observed_price": observed_price, "single_volume": single_volume,
        "total_volume": total_volume, "book_complete": book_complete, "source_gap": source_gap,
        "DQ_STATUS": dq_status, "SIGNAL_ELIGIBLE": signal_eligible,
    }
    row.update(levels)
    return row


def clean_records(records: Iterable[dict[str, Any]], *, run_id: str | None = None, expected_as_of_date: date | None = None) -> CleanResult:
    run_id = run_id or f"A4_003_{datetime.now(TZ).strftime('%Y%m%dT%H%M%S')}_{uuid.uuid4().hex[:8]}"
    normalized: list[dict[str, Any]] = []
    input_records = 0
    for record in records:
        input_records += 1
        normalized.extend(normalize_record(x, run_id=run_id, expected_as_of_date=expected_as_of_date) for x in expand_input(record))
    normalized.sort(key=lambda r: (r.get("AVAILABLE_AT") or "", r.get("SOURCE") or "", r.get("code") or ""))

    deduped, seen = [], set()
    duplicate_count = 0
    for row in normalized:
        if row["DATA_KEY"] in seen:
            duplicate_count += 1
            continue
        seen.add(row["DATA_KEY"])
        deduped.append(row)
    ...
    for row in deduped:
        ...
        for flag in row["DQ_STATUS"].split("|"):
            dq_counts[flag] = dq_counts.get(flag, 0) + 1

    return CleanResult(deduped, {
        "component": "A4_003", "version": VERSION, "run_id": run_id,
        "input_records": input_records, "expanded_rows": len(normalized), "output_rows": len(deduped),
        "exact_duplicates_removed": duplicate_count, "source_counts": source_counts, "dq_counts": dq_counts,
        "signal_eligible_rows": sum(r["SIGNAL_ELIGIBLE"] for r in deduped),
        "source_gap_rows": sum(r["source_gap"] for r in deduped), "pass": True,
    })
...
def read_ndjson(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for lineno, line in enumerate(handle, 1):
            ...
            obj = json.loads(line)
            if not isinstance(obj, dict): raise ValueError(f"line {lineno}: JSON object required")
            rows.append(obj)
    return rows
...
def self_test() -> dict[str, Any]:
    ...
    twse_item = {
        "ex":"tse","c":"2330","d":"20260817","t":"08:45:00","ts":"1","z":"1234.5","tv":"10","v":"100",
        "b":"1234_1233.5_1233_1232.5_1232_","g":"10_20_30_40_50_",
        "a":"1235_1235.5_1236_1236.5_1237_","f":"11_21_31_41_51_",
    }
    twse_env = {"captured_at":captured,"phase":"PREOPEN","raw":{"rtcode":"0000","msgArray":[twse_item]}}
    shioaji = {"source":"SHIOAJI","captured_at":"2026-08-17T08:45:00.600+08:00","event_type":"bidask_stk","exchange":"TSE","code":"2330","market_date":[2026,8,17],"market_time":[8,45,0,550000],"simtrade":True,"close":1234.5,"volume":10,"total_volume":100}
    for i in range(1,6):
        shioaji[f"bid_price_{i}"] = 1234.5-i*.5; shioaji[f"bid_volume_{i}"] = i*10
        shioaji[f"ask_price_{i}"] = 1234.5+i*.5; shioaji[f"ask_volume_{i}"] = i*11
        shioaji[f"diff_bid_vol_{i}"] = i; shioaji[f"diff_ask_vol_{i}"] = -i
    ...
    partial_item = dict(twse_item); partial_item["b"] = "1234_"
    partial = clean_records([{"captured_at":captured,"phase":"PREOPEN","raw":{"msgArray":[partial_item]}}], run_id="SELFTEST", expected_as_of_date=d)
    stale_item = dict(twse_item); stale_item["d"] = "20260814"
    stale = clean_records([{"captured_at":captured,"phase":"PREOPEN","raw":{"msgArray":[stale_item]}}], run_id="SELFTEST", expected_as_of_date=d)
    future_item = dict(twse_item); future_item["d"] = "20260818"
    future = clean_records([{"captured_at":captured,"phase":"PREOPEN","raw":{"msgArray":[future_item]}}], run_id="SELFTEST", expected_as_of_date=d)
    source_gap = clean_records([{"captured_at":captured,"phase":"PREOPEN","dq_status":"SOURCE_GAP","raw":{"msgArray":[]}}], run_id="SELFTEST", expected_as_of_date=d)
    open_item = dict(twse_item); open_item["t"] = "09:05:00"
    open_phase = clean_records([{"captured_at":"2026-08-17T09:05:00.100+08:00","raw":{"msgArray":[open_item]}}], run_id="SELFTEST", expected_as_of_date=d)
    shuffled = clean_records([{**shioaji,"captured_at":"2026-08-17T08:46:00+08:00"}, twse_env], run_id="SELFTEST", expected_as_of_date=d)
    missing_symbol = dict(shioaji); missing_symbol["code"] = ""
    missing = clean_records([missing_symbol], run_id="SELFTEST", expected_as_of_date=d)
    shioaji_row = next(r for r in normal.rows if r["SOURCE"] == "SHIOAJI")
    ...
    checks = {
        ...
        "shioaji_diff_volume_preserved": shioaji_row["diff_bid_vol_5"]==5,
    }
    return {"component":"A4_003","version":VERSION,"mode":"self_test","checked_at":datetime.now(TZ).isoformat(),"checks":checks,"pass":all(checks.values())}


def main() -> int:
    p = argparse.ArgumentParser(); p.add_argument("--self-test", action="store_true"); p.add_argument("--input", type=Path); p.add_argument("--output", type=Path, default=Path("output/a4_003/clean.ndjson")); p.add_argument("--report", type=Path, default=Path("output/a4_003/report.json")); p.add_argument("--as-of-date", type=date.fromisoformat); p.add_argument("--run-id"); a = p.parse_args()
    if a.self_test:
        report = self_test(); print(json.dumps(report, ensure_ascii=False)); return 0 if report["pass"] else 2
    if a.input is None: p.error("--input is required unless --self-test is used")
    result = clean_records(read_ndjson(a.input), run_id=a.run_id, expected_as_of_date=a.as_of_date)
    write_ndjson(a.output, result.rows); a.report.parent.mkdir(parents=True, exist_ok=True); a.report.write_text(json.dumps(result.report, ensure_ascii=False, indent=2), encoding="utf-8"); print(json.dumps(result.report, ensure_ascii=False)); return 0


if __name__ == "__main__":
    try: sys.exit(main())
    except Exception as exc:
        print(json.dumps({"component":"A4_003","pass":False,"error":f"{type(exc).__name__}: {exc}"}, ensure_ascii=False), file=sys.stderr); sys.exit(99)
